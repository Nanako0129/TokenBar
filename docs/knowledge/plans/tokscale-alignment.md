...
|---|---|
| TokenBar main baseline | [`471a7f23`](https://github.com/Nanako0129/TokenBar/commit/471a7f239f0270b4ebfaed04894335c506d588d3), the rebase-merge result of new-source M21 PR #71 |
...
The shared-parser critical path through M21, the money-correctness M18 checkpoint, and the independent M19-A filesystem checkpoint are merged. M22 is the active source-lane integration checkpoint and makes M23 ready after merge; M25 is independently ready from M18. M19-A's bounded Windows replacement behavior remains an M26 dependency. M26 is the final Native architecture barrier and M19-B runs exactly once after it.
...
| M21 | M17 | Merged as PR #71 at `471a7f23`: add Kimi Code beside legacy Kimi through structural parser selection and `KIMI_CODE_HOME`; append Junie/OpenCodeReview as client IDs 31/32; preserve Junie provider-reported cost, prompt ownership, duration anchors, OpenCodeReview workspace metadata, and materialized/streaming/count/report parity | Kept schema 31 |
| M22 | M21 | Active implementation checkpoint: append Zcode as client ID 33; parse legacy JSONL and v2 SQLite/WAL; distinguish legacy overlap from modern NULL totals; start-anchor timestamps and earliest-turn ownership; share DB-first source-tagged exact authority, raw caching, WAL invalidation, and materialized/streaming/count/report parity | Keep schema 31 |
...
| [Issue #45](https://github.com/Nanako0129/TokenBar/issues/45) | Designated public ledger; current through merged M21 at `74/13/0/10/13/1`; M22 moves four audited rows to `ALREADY_VENDORED` and mixed `b64d861e` to `DEFER` after merge |
