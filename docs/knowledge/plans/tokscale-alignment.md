...
| M23 (PR #74 open) | Parser production LOC ~`2.3x` upstream Desktop+VS Code parsers (`~1032` vs `451`: Desktop ~`300`, VS Code ~`730+`); total vendor-src churn higher (~`4x+` including hermetic fixtures, multi-lane suppress, scanner/fingerprint, and eight+ Codex review rounds). Named local seams: OTEL/Desktop/VS Code suppress on streaming+count+materialized with **date-filter before OTEL→Desktop and Desktop→VS Code session suppress**, force-retain Desktop+OTEL suppressors when VS Code survives mtime prune, VSCodium/Insiders/COPILOT_HOME/extra-root/`emptyWindowChatSessions`/legacy `.json` discovery, VS Code cache buckets via shared `normalize_input_tokens`, mutation-log kind:2 path replay, nanoAIU/credits cost authority, object `workspace.json`, UNC `file://` authority decode | Within adoption threshold: growth is TokenBar multi-lane/cache parity and selective discovery completeness, not a new authority algorithm. Keep selected; freeze further discovery invention; do not re-open Zcode-class invention |
