...
|---|---|
| TokenBar main baseline | M21 PR #71 at [`471a7f239f0270b4ebfaed04894335c506d588d3`](https://github.com/Nanako0129/TokenBar/commit/471a7f239f0270b4ebfaed04894335c506d588d3); M25 PR #75 at [`97c68d12981394dadccdb29b5f79503ad6a2eec9`](https://github.com/Nanako0129/TokenBar/commit/97c68d12981394dadccdb29b5f79503ad6a2eec9) |
...
| 111-row classification | Post-M25 main base `75/7/0/15/13/1`; this M23 branch advances to `ALREADY_VENDORED 79`, `TAKE 3`, `ADAPT_FOR_STREAMING 0`, `DEFER 15`, `SKIP 13`, `SUPERSEDED 1` |
| Cache | Active monolithic schema is **32** after Grok `turn_completed.usage` (PR #77); M23 does not change schema; closed-unmerged M22 made no runtime change |
...
| M23 (PR #74 open) | Parser production LOC ~`2.3x` upstream Desktop+VS Code parsers (`~1032` vs `451`: Desktop ~`300`, VS Code ~`730+`); total vendor-src churn higher (~`4x+` including hermetic fixtures, multi-lane suppress, scanner/fingerprint, and **nine+ Codex review rounds**). Named local seams: OTEL/Desktop/VS Code suppress on streaming+count+materialized with **date-filter before OTEL→Desktop and Desktop→VS Code session suppress**, force-retain Desktop+OTEL suppressors when VS Code survives mtime prune, VSCodium/Insiders/COPILOT_HOME/extra-root/`emptyWindowChatSessions`/legacy `.json` discovery, **extra-root pure-legacy `.json` promotion**, Desktop optional `total_nano_aiu` column probe, mutation-log kind:1+kind:2 path replay under `requests`, VS Code cache buckets via shared `normalize_input_tokens`, nanoAIU/credits cost authority (**AI credit → USD `*0.01`**; details parse only with credit/AIU/`$` labels), Desktop `session.start` selectedModel / `assistant.usage` model enrichment, object `workspace.json`, UNC `file://` authority decode | Within adoption threshold: growth is TokenBar multi-lane/cache parity and selective discovery/format completeness, not a new authority algorithm. Keep selected; **document freeze** — no more discovery/format expansion unless real-user bug; do not re-open Zcode-class invention |
...
The shared-parser critical path through M17, the money-correctness M18 checkpoint, the independent M19-A filesystem checkpoint, and M21 are merged. M22 is closed unmerged and deferred under the fidelity threshold; M23 depends on M21 directly and does not depend on M22. M25 is merged as PR #75 from M18 (post-M25-only ledger `75/7/0/15/13/1`). M23 lands in this worktree on top of that main base and advances the ledger to `79/3/0/15/13/1`. M24 can attach to the shared invalidation seam. M19-A's bounded Windows replacement behavior remains an M26 dependency. M26 is the final Native architecture barrier and M19-B runs exactly once after it.
