...
| 規則 | 做法 |
|---|---|
| Branch naming | 使用 `<type>/<kebab-summary>`，`type` 是這條分支主要 concern 的 Conventional Commit 前綴——也就是 PR 標題會帶的那個（`feat`、`fix`、`docs`、`chore`、`refactor`、`test`、`ci`、`build`、`perf`，或這個 repo 的 commit 已在用的其他前綴，清單不封閉），例如 `fix/registry-sleep-cancellation` 或 `docs/knowledge-base`；分支上的輔助 commit 可以用別的 type；Syrtis-Windows 與 tokscale-core 同一條規則。既有的 `<type>-<summary>` 分支不改名。 |
