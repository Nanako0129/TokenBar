...
⚠️ 本次**不是 pin-only**：`crates/tb_core_ffi` 需要改一行，因為 `local_cost_estimate` 以窮盡形式建構 `TokenBreakdown` literal，必須指名新欄位（填 0；`ModelUsage` 沒有 1h 桶，該估算本來就以 5m 費率估，門檻是 50 倍，不受影響）。這不是 `ctb.h` 簽名變更，故 Windows 不是本次簽名規則下的必須通知消費者；但 Windows 推進自己的 pin 時會需要同一行填值。
...
| Consumer adoption | Windows adopted this attribution revision in [PR #20](https://github.com/Nanako0129/TokenBar-Windows/pull/20), merge `eb3a7f3`; Native has since advanced independently to reviewed pin `65511b8172a6835df8670abe487807d970ee71e8`. |
...
The immutable implementation ledger for the Windows-adopted attribution revision remains [`UPSTREAM.md` at `84e0d66`](https://github.com/Nanako0129/tokscale-core/blob/84e0d66413d4e0d87b734f66f7a848b3bc323258/UPSTREAM.md); the current Native pin's exact engine ledger is [`UPSTREAM.md` at `65511b8`](https://github.com/Nanako0129/tokscale-core/blob/65511b8172a6835df8670abe487807d970ee71e8/UPSTREAM.md). The Windows consumer migration reviewed the complete engine delta from `b31e394` and ran its normal gates: hosted x64 and ARM64 builds, packaged-FFI, and the 119-case cross-check, plus a same-snapshot comparison in which `totalTokens` stayed byte-identical at 333,370,649 while the model bucket count moved 4 → 3. That adoption was limited to the reviewed gitlink advance. The current pin is not: `crates/tb_core_ffi` needs one line, because `local_cost_estimate` builds a `TokenBreakdown` literal exhaustively and must name the engine's new `cache_write_1h` field (see the current-pin paragraph above). No app-owned ABI change travels with it and the `ctb.h` signature is unchanged, so the Windows port is not a notified consumer under that rule for this advance — but **advancing the Windows pin will require the same one-line fill in its own FFI crate**, because the struct literal there is exhaustive too. Its crosscheck oracle pins the fixed macOS SHA `63084b2a` rather than `main`, so this advance cannot turn that job red; the oracle's own drift does grow, and advancing the Windows pin later will surface what accumulated in between.
