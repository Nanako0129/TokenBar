...
This rule applies to JSONL journals, Roo-family history, SQLite WAL files, Claude parent/workflow transcripts, and other secondary sources. A dedicated SQLite field such as Zcode may be intrinsically outside file-list pruning, but it still must use the composite database/WAL fingerprint in every cached lane and expose the WAL through both latest-mtime and topology-sensitive source tokens. A local adaptation is incomplete when only the parser or only the cache loader changes.
