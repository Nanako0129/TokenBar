...
5. 清快取這一步本身會失敗（可能回報 `Directory not empty`，本次量測就發生過兩次）。那一次**不是純冷**，要標記為污染，並在統計時同時給含入與排除兩種算法，不擇一呈現
...
冷啟動的兩個區間**完全不重疊**（NEW 最大 26,117 < OLD 最小 29,769）。其中兩次的清快取步驟回報失敗，排除那兩次重算為 −27.8%，結論不變。
