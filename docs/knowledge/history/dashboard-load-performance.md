...
1. 每對 OLD／NEW 用同一份語料與同一組環境
2. 每個 arm 從等價的空 message-cache 狀態開始
3. **交錯**執行（NEW/OLD、OLD/NEW…）以抵銷漂移，本次每組 7 對
4. **任一 arm 失敗，整對作廢**——配對的意義就是抵銷漂移，留下落單的觀測會直接偏移比較
5. 清快取這一步本身會失敗（可能回報 `Directory not empty`，本次量測就發生過兩次）。那是**arm 準備失敗**，不是一個比較髒的樣本：它違反第 2 點的等價空快取前提，而配對抵銷不了不對稱的快取命中。**整對作廢**，失敗要記錄下來但不進計算
6. 記錄語料識別、binary SHA、環境與熱狀態
