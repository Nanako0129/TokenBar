...
計時可以容忍語料漂移，**digest 不行**。本專案的開發過程本身在寫 log，所以正確性對拍必須跑在不可變的語料副本上。
...
**這組計時沒有跑在凍結語料上**——當時只有正確性對拍用了不可變副本。實際漂移量得出來：整組 14 個 arm 之間 `messages` 從 171,974 走到 171,979（+5，約 0.003%），每個 arm 的 corpus token 都不同。相對於 −27% 的效應可以忽略，但那是事後量出來的，不是設計時保證的；上面的協定已改成計時也要求不可變語料。
